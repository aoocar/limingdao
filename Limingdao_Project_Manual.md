...
为了保护用户隐私和增强由于直接跳出带来的体验割裂感，全站所有卡片的“直达跳出”功能均已接入中间确认页。

### 7.1 实现原理
*   **前端拦截与重定向**：所有的直达链接均已被修改为 `/redirect/?target={url}` 形式。
*   **静态跳转页**：在 `static/redirect/index.html` 实现纯前端渲染。
*   **安全拦截**：校验目标链接必须为 `http:` 或 `https:` 协议，防御 XSS 及伪造的 `javascript:` 漏洞。
*   **倒计时逻辑**：自带 5 秒强制倒计时，倒计时结束后解除按钮锁定。
